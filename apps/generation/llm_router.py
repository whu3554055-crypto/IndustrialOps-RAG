...
def _client_for_backend(backend: str | None = None) -> tuple[AsyncOpenAI, str]:
    ...
    if backend == "vllm":
        return AsyncOpenAI(base_url=s.vllm_base_url, api_key="EMPTY"), s.vllm_model
    if backend == "tensorrt_llm":
        return AsyncOpenAI(base_url=s.tensorrt_llm_base_url, api_key="EMPTY"), s.vllm_model
    if backend == "api":
        return AsyncOpenAI(), s.vllm_model  # 使用 OPENAI_* 环境变量
